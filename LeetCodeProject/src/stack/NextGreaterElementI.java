package stack;

import java.util.Stack;

/**
 * Problem description: You are given two arrays (without duplicates) nums1 and
 * nums2 where nums1’s elements are subset of nums2. Find all the next greater
 * numbers for nums1's elements in the corresponding places of nums2.
 * 
 * The Next Greater Number of a number x in nums1 is the first greater number to
 * its right in nums2. If it does not exist, output -1 for this number.
 * 
 * Example 1: 
 *  Input: 
 *  nums1 = [4,1,2], nums2 = [1,3,4,2]. 
 *  Output: [-1,3,-1]
 *  Explanation: For number 4 in the first array, you cannot find the next
 * greater number for it in the second array, so output -1. For number 1 in the
 * first array, the next greater number for it in the second array is 3. For
 * number 2 in the first array, there is no next greater number for it in the
 * second array, so output -1. 
 * Example 2: 
 *  Input: nums1 = [2,4], nums2 =[1,2,3,4]. 
 *  Output: [3,-1] 
 *  Explanation: For number 2 in the first array, the
 * next greater number for it in the second array is 3. For number 4 in the
 * first array, there is no next greater number for it in the second array, so
 * output -1.
 * Note: 
 *    All elements in nums1 and nums2 are unique. 
 *    The length of both nums1 and nums2 would not exceed 1000.
 * 
 * @author hgf
 * 
 */
public class NextGreaterElementI {
	
    public static void main(String[] args) {
    	int[] nums = {4, 3, 2, 1};
    	int[] findNums = {2, 4};
    	NextGreaterElementI nge = new NextGreaterElementI();
    	int[] result = nge.nextGreaterElementWithStack(findNums, nums);
    	for (int i : result) {
    		System.out.print(i + "\t");
    	}
    }
    
    // general algorithm without stack.
	public int[] nextGreaterElement(int[] findNums, int[] nums) {
		int[] result = new int[findNums.length];
		for (int i = 0; i < findNums.length; i++) {
			int pos = findPos(findNums[i], nums);
			result[i] = -1;
			for (int j = pos; j < nums.length; j++) {
				if (nums[j] > findNums[i]) {
					result[i] = nums[j];
					break;
				}
			}
		}
		return result;
    }
	
	public int findPos(int target, int[] nums) {
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == target)
				return i;
		}
		return -1;
	}
	
	public int[] nextGreaterElementWithStack(int[] findNums, int[] nums) {
		int[] result = new int[findNums.length];
		int[] nextGreater = new int[nums.length];
		for (int i = 0; i < nextGreater.length; i++) {
			nextGreater[i] = -1;
		}
		Stack<Integer> stack = new Stack<Integer>();
		for (int i = 0; i < nums.length; i++) {
			while (!stack.empty() && nums[stack.peek()] < nums[i]) {
				nextGreater[stack.pop()] = nums[i];
			}
			stack.push(i);
		}
		for (int i = 0; i < findNums.length; i++) {
			int pos = findPos(findNums[i], nums);
			result[i] = nextGreater[pos];
		}
		return result;
	}
}
